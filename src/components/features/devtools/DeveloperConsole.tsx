...
export function DeveloperConsole({ onVisibilityChange }: DeveloperConsoleProps = {}) {
  ...
  if (!isVisible) {
    return (
      <button
        type='button'
        className='fixed bottom-4 left-4 flex h-11 w-11 cursor-pointer items-center justify-center rounded-xl border border-white/10 bg-black/55 text-gray-300 opacity-90 backdrop-blur-sm transition-all hover:scale-105 hover:opacity-100'
        onClick={() => {
          setIsVisible(true)
          // Mark dev console as opened (enables achievement system)
          localStorage.setItem('dev_console_opened', 'true')
        ...
        style={{
          // Keep clear of Next.js dev-tools dock in development.
          bottom: isDevelopment ? '5.5rem' : undefined,
          zIndex: 120,
          // iOS safe area
      ...
      >
        <Terminal className='h-6 w-6 text-gray-400' aria-hidden='true' />
      </button>
    )
  }
  ...
  return (
    <>
      {/* Mobile: Compact non-blocking panel */}
      {isMobile && (
        <div className='pointer-events-none fixed bottom-16 left-0 w-full px-4' style={{ zIndex: 70 }}>
          <div className='mx-auto w-full max-w-sm'>
            <div
              className='pointer-events-auto relative flex flex-col overflow-hidden rounded-lg border-2 backdrop-blur-xl'
              role='region'
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  className='flex h-8 w-8 cursor-pointer items-center justify-center text-2xl text-gray-400 transition-colors hover:text-white'
      ...
      {!isMobile && (
        <div
          ref={panelRef}
          className={`fixed font-mono text-xs transition-all ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
        ...
        >
          ...
          >
            ...
            {/* Content */}
            <div className='min-w-[280px] space-y-2 p-4'>{renderConsoleContent()}</div>
          </div>
        </div>
      )}
    </>
